...
import org.chargers.frc2018.subsystems.Subsystem;
...
public class DriveTrain extends Subsystem {
	...
	public void autoPeriodic() {
		// Motor safety feature, stops the robot if the motor output has not
		// been updated
	 ...
	 * 
	 * @param forwards
	 *            The amount of movement in the y-axis direction, relative to
	 *            the field, input in the range of -1 to 1
	 * @param sideways
	 *            The amount of movement in the x-axis direction, relative to
	 *            the field, input in the range of -1 to 1
	 * @param rotation_x
	 *            The x value of the rotation joystick, input in the range of -1
	 *            to 1
	 * @param rotation_y
	 *            The y value of the rotation joystick, input in the range of -1
	 *            to 1
	 */
	public void mecanumDriveField(double forwards, double sideways, double rotation_x, double rotation_y) {
		...
		// clockwise
		double angle_error = Math
				.toDegrees(Math.asin(Math.sin(Math.toRadians(this.getAngle()) - Math.toRadians(desired_angle))));

		double rotation = -turnPID.runPID(angle_error, 0);// Plug the error into
															// PID to get to the
															// desired angle
		...
		double temp = forwards * Math.sin(Math.toRadians(this.getAngle()))
				+ sideways * Math.cos(Math.toRadians(this.getAngle()));
		sideways = -forwards * Math.cos(Math.toRadians(this.getAngle()))
				+ sideways * Math.sin(Math.toRadians(this.getAngle()));
		forwards = temp;
	 ...
	 * @param forwards
	 *            The amount of movement in the y-axis direction, relative to
	 *            the robot, input in the range of -1 to 1. Higher values will
	 *            work but won't lead to faster speeds
	 * @param sideways
	 *            The amount of movement in the x-axis direction, relative to
	 *            the robot, input in the range of -1 to 1. Higher values will
	 *            work but won't lead to faster speeds
	 * @param rotation
	 *            The amount of power in rotation, relative to the robot, input
	 *            in the range of -1 to 1. Higher values will work but won't
